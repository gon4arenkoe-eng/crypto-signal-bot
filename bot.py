"""
Crypto Signal Bot v11.2 -- Multi-Timeframe EMA Strategy + Paper Trading + Charts
Kraken API + Yahoo Finance fallback | Render-ready
"""
...
from paper_trading import get_paper_engine, PaperTradingDB, TradeStatus
...
def send_photo_bytes(chat_id, photo_bytes, caption="", parse_mode="HTML"):
    ...
    try:
        ...
        logger.error(f"sendPhoto HTTP {resp.status_code}: {resp.text[:200]}")
    except Exception as e:
...
def handle_start(chat_id, user):
    ...
    msg = "👋 Hello, " + fname + "!\n\n🤖 Crypto Signal Bot v11.2\nMulti-Timeframe EMA Strategy + Paper Trading\n4h Trend + 1h EMA Crossover\n\nChoose action 👇"
...
def handle_top(chat_id):
    ...
    for symbol in coins:
        ...
        # Получаем изменение за 24ч через OHLCV
...
# ===== NEW PAPER TRADING COMMANDS =====

def handle_paper_toggle(chat_id):
    """Вкл/выкл paper trading"""
...
💵 Текущий баланс: <code>${balance:,.2f}</code>

<i>При ВЫКЛЮЧЕННОМ paper trading бот будет отправлять реальные ордера!</i>

Используй:
/portfolio — открытые позиции
/stats — статистика + график
/chart — график эквити
...
def handle_stats(chat_id):
    ...
    try:
        ...
        send_photo_bytes(
            chat_id, png_bytes,
            caption="📊 <b>Equity Curve</b> — 30 дней",
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Chart generation error: {e}")
        send_message(chat_id, "❌ Ошибка генерации графика")
...
def handle_chart(chat_id, args):
    ...
    send_message(chat_id, f"📈 Генерирую график за {days} дней...")
    try:
        png_bytes = chart_gen.generate_equity_chart(days)
        send_photo_bytes(
            chat_id, png_bytes,
            caption=f"📊 <b>Equity Curve</b> — {days} дней",
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Chart generation error: {e}")
...
def handle_reset(chat_id):
    ...
    send_message(
        ...
        "Баланс восстановлен: <code>$10,000.00</code>\n"
        "Вся история очищена.\n\n"
        "<i>Начни торговлю заново</i>"
    )


def handle_old_stats(chat_id):
    """Старая статистика сигналов (сохраняем)"""
    conn = get_db()
    c = conn.cursor()
...
def process_paper_signal(signal):
    """
    Обрабатывает сигнал для paper trading.
    Вызывается из auto_scanner при получении сигнала.
    """
    if not paper_db.is_paper_enabled():
        logger.info("Paper trading disabled, skipping signal")
        return
    ...
    if side == "BUY":
        ...
        trade = paper_engine.open_position(
            ...
            take_profit=signal["take_profit_1"],  # TP1 как основной
            risk_percent=2.0
        )

        if trade:
            logger.info(f"Paper trade opened: {symbol} LONG @ ${price}")
            # Отправляем уведомление всем пользователям
            conn = get_db()
            c = conn.cursor()
            c.execute("SELECT chat_id FROM users WHERE auto_signals = 1")
            users = c.fetchall()
            conn.close()
            ...
            msg = (
                ...
                f"Risk: <code>2%</code>\n\n"
                f"Strategy: EMA_Cross (1h)"
            )
            for (chat_id,) in users:
                send_message(chat_id, msg)

    # Для SELL — закрываем позицию
    elif side == "SELL":
        open_trades = paper_db.get_open_trades(symbol)
        for trade in open_trades:
            ...
            if closed:
                ...
                logger.info(f"Paper trade closed: {symbol} @ ${price} P&L: {closed.pnl:+.2f}")
                ...
                msg = (
                    ...
                    f"P&L: <code>${closed.pnl:,.2f}</code> ({closed.pnl_percent:+.2f}%)\n\n"
                    f"New Balance: <code>${paper_db.get_balance():,.2f}</code>"
                )
                for (chat_id,) in users:
                    send_message(chat_id, msg)
...
def check_paper_stops_auto():
    """Проверка стоп-лоссов и тейк-профитов для paper trading"""
    if not paper_db.is_paper_enabled():
        return

    # Получаем все уникальные символы с открытыми позициями
    ...
    for symbol in symbols:
        ...
        for trade in closed:
            ...
            msg = (
                f"📊 <b>PAPER {trade.close_reason}</b> {reason_emoji}\n\n"
                f"{pnl_emoji} <b>{trade.symbol}</b> закрыта\n"
                f"Цена: <code>${trade.exit_price:,.2f}</code>\n"
                f"P&L: <code>${trade.pnl:,.2f}</code> ({trade.pnl_percent:+.2f}%)\n\n"
                f"Balance: <code>${paper_db.get_balance():,.2f}</code>"
            )
            for (chat_id,) in users:
                send_message(chat_id, msg)
...
@app.route("/")
def home():
    return "Bot v11.2 Multi-Timeframe EMA + Paper Trading running!"
