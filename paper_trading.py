...
class PaperTradingEngine:
    ...
    def format_stats_message(self) -> str:
        ...
        total_return = ((p.balance / p.initial_balance - 1) * 100)

        message = "📊 <b>Paper Trading Statistics</b>\n\n"
        message += emoji_balance + " <b>Balance</b>\n"
        message += "   Initial: <code>$" + f"{p.initial_balance:,.2f}" + "</code>\n"
        message += "   Current: <code>$" + f"{p.balance:,.2f}" + "</code>\n"
        message += "   Peak: <code>$" + f"{p.peak_balance:,.2f}" + "</code>\n\n"

        message += emoji_pnl + " <b>P&L</b>\n"
        message += "   Total: <code>$" + f"{p.total_pnl:,.2f}" + "</code> (" + f"{total_return:+.2f}" + "%)\n"
        message += "   Current DD: <code>" + f"{p.current_drawdown:.2f}" + "%</code>\n"
        message += "   Max DD: <code>" + f"{p.max_drawdown_percent:.2f}" + "%</code>\n\n"

        message += "📋 <b>Trades</b>: " + str(p.total_trades) + "\n"
        message += "   ✅ Wins: " + str(p.winning_trades) + " (" + f"{stats['winrate']:.1f}" + "%)\n"
        message += "   ❌ Losses: " + str(p.losing_trades) + "\n"
        message += "   🔄 Open: " + str(p.open_positions) + "\n\n"

        message += "⚡ <b>Metrics</b>\n"
        message += "   Profit Factor: <code>" + f"{stats['profit_factor']:.2f}" + "</code>\n"
        message += "   Sharpe Ratio: <code>" + f"{stats['sharpe_ratio']:.2f}" + "</code>\n"
        message += "   Avg Win: <code>$" + f"{stats['avg_win']:,.2f}" + "</code>\n"
        message += "   Avg Loss: <code>$" + f"{stats['avg_loss']:,.2f}" + "</code>"

        if stats["best_trade"]:
            message += "\n   Best: <code>+$" + f"{stats['best_trade'].pnl:,.2f}" + "</code> (" + stats["best_trade"].symbol + ")"
            message += "\n   Worst: <code>$" + f"{stats['worst_trade'].pnl:,.2f}" + "</code> (" + stats["worst_trade"].symbol + ")"
            message += "\n   Avg Duration: <code>" + f"{stats['avg_trade_duration']:.1f}" + "h</code>"
    ...
    def format_portfolio_message(self) -> str:
        ...
        if not open_trades:
            return "📭 <b>No open positions</b>\n\n💵 Paper Balance: <code>$" + f"{balance:,.2f}" + "</code>"

        message = "📂 <b>Open Positions</b> (" + str(len(open_trades)) + ")\n"
        message += "💵 Balance: <code>$" + f"{balance:,.2f}" + "</code>\n\n"

        for trade in open_trades:
            emoji = "🟢" if trade.side == "LONG" else "🔴"
            sl = "\n   SL: <code>$" + f"{trade.stop_loss:,.2f}" + "</code>" if trade.stop_loss else ""
            tp = "\n   TP: <code>$" + f"{trade.take_profit:,.2f}" + "</code>" if trade.take_profit else ""

            message += emoji + " <b>" + trade.symbol + "</b> | " + trade.side + "\n"
            message += "   Entry: <code>$" + f"{trade.entry_price:,.2f}" + "</code> | Qty: <code>" + f"{trade.quantity:.6f}" + "</code>\n"
            message += "   Time: <code>" + trade.entry_time[:16].replace("T", " ") + "</code>\n"
            message += "   Strategy: <code>" + trade.strategy + "</code> (" + trade.timeframe + ")" + sl + tp + "\n\n"
    ...
    def format_history_message(self, limit: int = 10) -> str:
        ...
        message = "📜 <b>Last " + str(len(trades)) + " Closed Trades</b>\n\n"

        for trade in trades:
            emoji = "🟢" if trade.pnl and trade.pnl > 0 else "🔴"
            reason = " (" + trade.close_reason + ")" if trade.close_reason else ""

            message += emoji + " <b>" + trade.symbol + "</b> | " + trade.side + reason + "\n"
            message += "   Entry: <code>$" + f"{trade.entry_price:,.2f}" + "</code> → Exit: <code>$" + f"{trade.exit_price:,.2f}" + "</code>\n"
            message += "   P&L: <code>$" + f"{trade.pnl:,.2f}" + "</code> (" + f"{trade.pnl_percent:+.2f}" + "%)\n"
            message += "   Time: <code>" + trade.entry_time[:16].replace("T", " ") + "</code> → <code>" + trade.exit_time[:16].replace("T", " ") + "</code>\n\n"
