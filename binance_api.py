...
_STRATEGY_PARAMS_CACHE = None

def get_strategy_params(symbol: str = "BTC") -> dict:
    """Получает параметры стратегии из оптимизатора или дефолтные"""
    global _STRATEGY_PARAMS_CACHE
    if _STRATEGY_PARAMS_CACHE is not None:
        return _STRATEGY_PARAMS_CACHE

    try:
        import sqlite3, json
        conn = sqlite3.connect("bot_config.db")
        c = conn.cursor()
        c.execute("SELECT params_json FROM optimized_params WHERE symbol = ? AND is_active = 1 ORDER BY optimized_at DESC LIMIT 1", (symbol,))
        row = c.fetchone()
        conn.close()
        if row:
            _STRATEGY_PARAMS_CACHE = json.loads(row[0])
            logger.info("Using OPTIMIZED strategy params")
            return _STRATEGY_PARAMS_CACHE
    except Exception as e:
        logger.warning(f"Could not load optimized params: {e}")

    _STRATEGY_PARAMS_CACHE = {
        "ema_fast": 8, "ema_slow": 50, "ema_trend": 50,
        "sl_multiplier": 2.5, "tp1_multiplier": 1.5,
        "tp2_multiplier": 3.0, "tp3_multiplier": 5.0,
        "risk_per_trade": 2.0
    }
...
def analyze_binance_signal(symbol="BTCUSDT", interval="1h", params: dict = None):
    fsym = _resolve_symbol(symbol)

    # Получаем параметры стратегии
    if params is None:
        params = get_strategy_params(fsym)

    ema_fast = params.get("ema_fast", 8)
    ema_slow = params.get("ema_slow", 50)
    ema_trend = params.get("ema_trend", 50)
    sl_mult = params.get("sl_multiplier", 2.5)
    tp1_mult = params.get("tp1_multiplier", 1.5)
    tp2_mult = params.get("tp2_multiplier", 3.0)
    tp3_mult = params.get("tp3_multiplier", 5.0)
    ...
    ema_trend_4h = calculate_ema(closes_4h, ema_trend)

    current_close = closes_1h[-1]
    trend_bullish = current_close > ema_trend_4h[-1]
    trend_bearish = current_close < ema_trend_4h[-1]

    ema_cross_up = (ema_fast_1h[-2] <= ema_slow_1h[-2] and ema_fast_1h[-1] > ema_slow_1h[-1])
    ema_cross_down = (ema_fast_1h[-2] >= ema_slow_1h[-2] and ema_fast_1h[-1] < ema_slow_1h[-1])
    ...
    if ema_cross_up and volume_confirmed and trend_bullish:
        ...
        if current_volume > avg_volume * 1.5:
            signal_strength += 1
        if current_close > ema_trend_4h[-1] * 1.02:
            signal_strength += 1
        ...
        if current_volume > avg_volume * 1.5:
            signal_strength += 1
        if current_close < ema_trend_4h[-1] * 0.98:
            signal_strength += 1
    ...
    if signal == "BUY":
        sl = min(current_close - atr * sl_mult, current_close * 0.975, support * 0.998)
        ...
        return {
            ...
            "take_profit_1": round(current_close + risk * tp1_mult, 2),
            "take_profit_2": round(current_close + risk * tp2_mult, 2),
            "take_profit_3": round(current_close + risk * tp3_mult, 2),
            "risk_reward": 1.5, "stars": stars, "rsi": round(rsi_1h, 2),
            "change_24h": round(change_24h, 2), "price": current_close,
            "ema8": round(ema_fast_1h[-1], 2), "ema50": round(ema_slow_1h[-1], 2),
            "ema50_4h": round(ema_trend_4h[-1], 2), "volume": round(current_volume, 4),
            "trend": "BULLISH" if trend_bullish else "BEARISH",
        }
    else:
        sl = max(current_close + atr * sl_mult, current_close * 1.025, resistance * 1.002)
